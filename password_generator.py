...
def validation(rules):
    if password_length == '':
        print(f'{bcolors.FAIL}No password length was defined.')
        exit()
    elif selected_characters == '':
        print(f'{bcolors.FAIL}No character categories were defined.')
        exit()
    elif password_length > rules['max_length']:
        print('A password length of ' + password_length + ' is not allowed.')
        print('The maximum password length is ' + rules['max_length'] + ' characters.')
        exit()
    else:
